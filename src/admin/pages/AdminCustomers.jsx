...
import { Mail, Loader2, RefreshCw, Send, X, CheckCircle2 } from "lucide-react";
import { toast } from "sonner";
import { customers as seed, money } from "@/shared/lib/products";
...
function AdminCustomers() {
    ...
    try {
      ...
      if (res.customers && res.customers.length > 0) {
        const mapped = res.customers.map(c => ({
          ...
          tier: c.tier || "Bronze",
          joined: c.joined ? new Date(c.joined).toISOString().split("T")[0] : "2026-08-18"
        }));
        setRows(mapped);
      } else {
        setRows(seed);
      ...
      console.warn("Using fallback customers:", err.message);
      setRows(seed);
    } finally {
      setLoading(false);
    }
  ...
  const handleOpenEmailModal = (customer) => {
    ...
    setSubject("Exclusive Update from Lumière Aura Atelier");
    setEmailBody(`Dear ${customer.name},\n\nWe appreciate you being a valued client of Lumière Aura Atelier. `);
    setIsEmailModalOpen(true);
  };

  const handleSendCustomerEmail = async (e) => {
    e.preventDefault();
    if (!emailBody.trim()) {
  ...
  return (
    <div className="grid gap-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl">Registered Clients & Customers</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {rows.length} registered clients live in PostgreSQL database.
          </p>
        </div>
        <button
      ...
      {loading && (
        <div className="py-12 text-center text-muted-foreground">
          ...
          Loading registered customers from PostgreSQL...
        </div>
      )}
              ...
              <div>
                ...
                <dl className="mt-5 grid grid-cols-3 gap-2 text-center text-xs">
                  <div>
                    ...
                    <dd className="mt-1 text-base font-semibold text-foreground">{c.orders}</dd>
                  </div>
                  <div>
                    <dt className="text-muted-foreground">Spent</dt>
                    <dd className="mt-1 text-base font-semibold text-accent">{money(c.spent)}</dd>
                  </div>
                  <div>
                    <dt className="text-muted-foreground">Joined</dt>
                    <dd className="mt-1 text-base font-semibold text-foreground">{c.joined.slice(0, 7)}</dd>
                  </div>
                </dl>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg rounded-2xl bg-card p-6 shadow-2xl border border-border">
            <div className="flex items-center justify-between border-b border-border pb-4">
              <div>
                <h2 className="font-display text-xl">Email {activeCustomer.name}</h2>
                <p className="text-xs text-muted-foreground">
                  Sending from <span className="text-accent font-semibold">rabiamukhtar5948@gmail.com</span> to <span className="font-semibold text-foreground">{activeCustomer.email}</span>
                </p>
              </div>
            ...
            <form onSubmit={handleSendCustomerEmail} className="mt-4 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Special Invitation / Order Update"
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Write your email to the client here..."
                />
              </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  ...
                  className="rounded-lg bg-royal px-5 py-2 text-xs uppercase tracking-wider text-primary-foreground shadow-glow hover:opacity-90 disabled:opacity-50 flex items-center gap-2 cursor-pointer"
                >
                  {sendingEmail ? (
                    <>
